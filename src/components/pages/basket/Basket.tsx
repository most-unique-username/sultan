...
import { ProductBoxBasket } from '../../ProductBoxBasket';
import { getPurchaseSum } from '../../../functions/getPurchaseSum';
...
export const Basket = () => {
  ...
  const changeNumberProduct = (product: IProduct, quantity: number, difference: number): void => {
    quantity + difference === 0 ?
      dispatch({ type: BasketActionTypes.DELETE_PRODUCT, payload: product.vendorCode }) :
      dispatch({ type: BasketActionTypes.CHANGE_QUANTITY_PRODUCT, payload: { product: product, difference: difference } });
  }
    ...
    <div className='page-basket'>
      ...
      <Container containerClass='page-basket__container container'>
        ...
        {products.map(item =>
          ...
          <Container containerClass='page-basket__row'>
            ...
            <ProductBoxBasket
              ...
              boxClass="page-basket__product-box"
              productboxClass="page-basket__product-data"
              boximgClass="page-basket__product-box__img-box"
              imgClass="page-basket__product-box__img"
              rowClass='page-basket__box-row'
              iconClass="page-basket__icon"
              sizeClass='page-basket__product-box__size'
              nameClass="page-basket__product-box__name"
              descriptionClass="page-basket__product-box__description"
            />

            <Container containerClass='page-basket__price-box'>

              <ButtonCounter
                decrement={() => changeNumberProduct(item.product, item.quantity, -1)}
                increment={() => changeNumberProduct(item.product, item.quantity, 1)}
                quantity={item.quantity}
              />

              <p className='page-basket__price'>{`${getPurchaseSum(item.product.price, item.quantity)}₸`}</p>
        ...
        <Container containerClass='page-basket__row page-basket__row_noline'>
          ...
          <Button
            onClick={() => { clearBasket(); setOrder(true) }}
          >
            Оформить заказ
          </Button>
